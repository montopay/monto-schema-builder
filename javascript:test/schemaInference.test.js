...
test("should merge object properties with compatible types (integer/number)", () => {
  const json = {
    items: [
      { id: 1, value: 10 },
      { id: 2, value: 20.5 },
  ...
  // 'value' should be merged to 'number' as it contains both integers and floats
  assert.strictEqual(
    schema.properties.items.items.properties.value.type,
    "number",
  );
  assert.deepStrictEqual(schema.properties.items.items.required, [
    "id",
    "value",
  ]);
});
...
test("should detect coordinate arrays [lat, lon]", () => {
  ...
  const schema = createSchemaFromJson(json);
  // Test 'coordinates'
  const coordsSchema = schema.properties.locations.items.properties.coordinates;
...
test("should handle arrays where items merge into a single object type", () => {
  ...
  assert.ok(!schema.properties.values.items.oneOf, "Items should not be oneOf");
  assert.strictEqual(
